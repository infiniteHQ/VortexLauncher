...
VORTEX_API bool VortexMaker::CheckProjectInDirectory(const std::string &path) {
  if (!std::filesystem::exists(path) || !std::filesystem::is_directory(path)) {
    VortexMaker::LogError("Core", "Directory does not exist or is not accessible: " + path);
    return false;
  }

  std::filesystem::path modulePath = std::filesystem::path(path) / "vortex.config";
...
VORTEX_API void VortexMaker::FindpProjectsInDirectoryRecursively(
