...
# -----------------------------
rm -rf shipping
mkdir -p shipping/launcher/macos
...
if [ "$NO_INSTALLER" = false ]; then
  TAR_FILE=./shipping/launcher/macos/vortex_launcher_${VERSION}.tar.gz
  tar -cvzf "$TAR_FILE" dist

  cd shipping/launcher/macos
  ...
  cp shipping/launcher/macos/vortex_launcher_${VERSION}.tar.gz ../lib/installer/src/ui/assets/builtin/
  cp shipping/launcher/macos/vortex_launcher_${VERSION}.tar.gz.sha256 ../lib/installer/src/ui/assets/builtin/
...
EOL
  ...
  cp ../lib/installer/build/build/bin/VortexInstaller shipping/launcher/macos/
fi
